...
export default function EditTodoForm({
  ...
  const [input, setInput] = useState({
    date: postObj.date.split("T")[0],
    carId: postObj.car.id,
    driverId: postObj.driver.id,
    customerId: postObj.customer.id,
    origin: postObj.origin,
    destination: postObj.destination,
    detial: postObj.detial,
    price: postObj.price,
    userId: authUser.id,
  });
  ...
  return (
    <form className="flex flex-col gap-4 text-base" onSubmit={handleSubmitForm}>
      <div className="grid grid-cols-2 gap-4 ">
        ...
        <div className="flex flex-col">
          ...
          >
            {carCalls.map((carId) => {
              return (
                <option key={carId.name} value={carId.id}>
                  {carId.name}
                </option>
              );
            })}
        ...
        <div>
          ...
          >
            {driverNames.map((driverId) => {
              return (
                <option key={driverId.id} value={driverId.id}>
                  {driverId.name}
                </option>
              );
            })}
